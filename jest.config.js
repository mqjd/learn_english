...
const pkgs = glob.sync(`./plugins/*`).map((p) => p.replace(/^\./, `<rootDir>`));
...
const ignoreDirs = [`<rootDir>/plugins/gatsby-dev-cli/verdaccio`].concat(
  // gatsbyBuildDirs,
  builtTestsDirs,
  distDirs
);
...
const config = {
  ...
  testPathIgnorePatterns: [
    ...
    `<rootDir>/plugins/gatsby-plugin-gatsby-cloud/src/__tests__/mocks/`,
    `<rootDir>/plugins/gatsby/src/utils/worker/__tests__/test-helpers/`,
    `<rootDir>/deprecated-plugins/`,
    `__tests__/fixtures`,
    `__testfixtures__/`,
  ],
